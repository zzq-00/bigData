...
export default {
    ...
    var validateName = (rule, value, callback) => {
      if (value === '') {
        ...
        let data = {
          ...
          pid: this.projectId,
          sid: this.stageId,
          monomerId: this.aloneData.id ? this.aloneData.id : ''
        }
  ...
  methods: {
      ...
      if (
        ...
        this.$confirm('操作会将已分析过指标数据一并删除，是否确定删除此单项工程？', '提示', {
          ...
          .then(() => {
            ...
            api.DelMonomerMonomer(data).then(res => {
              this.$message({
                showClose: true,
                message: '删除成功',
                type: 'success'
        ...
        this.$confirm('是否删除当前单项工程信息？', '提示', {
          ...
          .then(() => {
            //确定删除
            api.DelMonomerMonomer(this.aloneData.id, this.ruleForm.indexCode).then(res => {
              this.$message({
                showClose: true,
                message: '删除成功',
                type: 'success'
    ...
    //复制
    duplicateFn() {
      api.monomerGetMonomer(this.aloneData.id, this.projectId).then(res => {
        this.detailsChangeData(res.data)
        this.$message({
          showClose: true,
          message: '复制成功',
          ...
          if (!this.oldData) {
            ...
            api
              ...
              .then(res => {
                ...
                api.saveMonomersOfficial(this.separateData(id, newVersion).data).then(res => {
            ...
            if (this.aloneData.tag) {
              ...
              api.saveMonomersOfficial(this.separateData(id).data).then(res => {
                this.ruleForm.id = res.data
                this.$message({
                  showClose: true,
                  message: '保存成功',
