<template>
  <div class="analysis-page">
    ...
    <div class="analysis-content">
      <stepBar v-bind:ctitle="title" v-bind:cStepList="stage" />
      <router-view class="analysis-stage" />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      title: '指标分析',
      stage: ['选择文件', '匹配单项工程', '填写单项工程信息', '指标分析完成'],
  ...
  mounted() {
    ...
    api.getProjectDetail({ projectId: this.projectId }).then(res => {
      ...
      this.breadcrumbs.push({ name: this.projectData.common.projectName, path: '' })
    })
  }
}
</script>
