<template>
  <div class="res-lists">
    <div class="proRig">
      <span class="total">共有 {{totalElements}} 条数据</span>
      <span v-show="totalElements" style="margin: 0 10px;">
        <el-button type="text" icon="el-icon-arrow-left" :disabled="singleItemFilterData.page==1" @click="paging('-')"></el-button>
        <span>{{singleItemFilterData.page}}/{{totalPages}} </span>
        <el-button type="text" icon="el-icon-arrow-right" :disabled="singleItemFilterData.page==totalPages" @click="paging('+')"></el-button>
      </span>
    </div>
    <nav class="nav-title">
      所有符合条件结果
      <!-- 还有activedown activeup两个class样式 -->
      <span :class="[singleItemFilterData.sort==='desc'?'up':'down', 'sort-icon sort-icon3']" @click="sortClick">价格排序</span>
    </nav>
    <ul class="lists" v-if="dataList.length" v-loading="dataWait">
    ...
    <div class="block">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="singleItemFilterData.page" :page-sizes="[10, 20, 30, 40]" :page-size.sync="singleItemFilterData.size" layout="sizes,prev, pager, next, jumper" :total="totalElements">
      </el-pagination>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    getSingleProjectByCondition(val) {
      ...
      api.getSingleProjectByCondition(val).then(res => {
        ...
        this.totalPages = Math.ceil(res.count / this.singleItemFilterData.size)
      })
    },
    // 页数
    handleCurrentChange(val) {
      this.setSingleItemFilterData({
        dataForBack: { page: val }
      })
    },
    // 每页数量
    ...
    // 排序
    sortClick() {
      if (this.singleItemFilterData.sort === 'asc') {
        this.setSingleItemFilterData({
          dataForBack: { sort: 'desc' }
        })
      } else {
        this.setSingleItemFilterData({
          dataForBack: { sort: 'asc' }
        })
      }
    },
    // 列表上部翻页
    paging(val) {
      let num = this.singleItemFilterData.page
      if (val == '-') {
        num--
      } else if (val == '+') {
        num++
