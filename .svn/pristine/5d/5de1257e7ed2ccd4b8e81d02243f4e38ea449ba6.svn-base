    ...
    <div class="container">
      <div class="side-menu" :class="{ sideMenus: isActive }" v-drag ref="sides">
        <el-menu :default-openeds="defaultsOpen" :default-active="defaultPath" @open="handleOpen" @close="handleClose" @select="handleSelect" :unique-opened="true" collapse-transition>
          <el-menu-item :index="`/projectLibrary/detail/projectInfo/${projectId}`" class="single-menu">
            <i class="iconfont icon-projectInformation"></i>
            <span class="titles" slot="title">项目信息</span>
          </el-menu-item>
          <el-submenu index="/projectLibrary/detail/singleInfo" class="index-menu single" v-if="stageList.length>0">
            <template slot="title">
              <i class="iconfont icon-dantixinxi"></i>
              <span class="titles">单项工程信息</span>
            </template>
            <el-menu-item :index="`/projectLibrary/detail/singleInfo/${projectId}/${item.stageId}`" v-for="(item,id) in stageList" :key="id" v-if="!isActive">
              {{item.name}}
            </el-menu-item>
          </el-submenu>
          <el-submenu index="/projectLibrary/detail/newIndexData" v-if="directoryList.length>0" class="index-menu">
            <template slot="title">
          ...
          <el-menu-item :index="`/projectLibrary/detail/fileInfo/${projectId}`" class="single-menu">
...
export default {
  ...
  data() {
    return {
      breadcrumbs: [{ name: '企业项目库', path: '/projectLibrary' }],
  ...
  methods: {
    shareGetProjectDocumentsStage() {
      api
        .shareGetProjectDocumentsStage(this.projectId)
        .then(res => {
    ...
    indicatorType(type, path) {
      ...
      return `/projectLibrary/detail/newIndexData/${indexType}/${this.projectId}${buildingId ? '/' + buildingId : ''}${
        stageId ? '/' + stageId : ''
      }`
    },
    // 指标数据层级目录
    sidebarDirectory() {
      api.entSidebar(this.projectId).then(res => {
    ...
    handleClose(key, keyPath) {
      if (key == '/projectLibrary/detail/newIndexData') {
        this.$router.push(`/projectLibrary/detail/projectInfo/${this.projectId}`)
      }
    },
    ...
    handleEditClick(value) {
      if (value) {
        this.getEnterpriseProjectDetail()
      }
    },
    // 获取项目详情
    getEnterpriseProjectDetail() {
      api.getEnterpriseProjectDetail({ projectId: this.projectId }).then(res => {
  ...
  mounted() {
    ...
    this.shareGetProjectDocumentsStage()
