...
export default {
  ...
  watch: {
    ...
    '$route.params.buildingId': {
      handler: function(val, oldVal) {
        this.loading = true
        this.monomerSummaryItemsSharing(val)
      },
      immediate: true
    }
  },
  ...
  methods: {
    ...
    load(tree, treeNode, resolve) {
      let data = []
      api.itemsByNineDigitSharing({ monomerId: this.buildingId, nineDigit: tree.data.code }).then(res => {
        data = res.data.data
        resolve(data)
      })
    },
    clickRow(row) {
      let routePath = this.$router.resolve(`/projectLibrary/comprehensive/${row.data.code}/${row.data.unitProjectId}`)
    ...
    monomerSummaryItemsSharing(val) {
      api
        .monomerSummaryItemsSharing({ monomerId: val })
        .then(res => {
          this.arr1 = []
          this.arr2 = []
          this.arr3 = []
