    ...
    <div class="common-choice">
        ...
        <transition>
            <ul class="lists no-warp block" :style="{height: height}" key="provice">
                <li class="list" v-for="item in struct" :key="item.id">
                    <input type="radio"
                        :id="'struct'+item.id"
                        v-bind="$attrs"
                        ...
                        v-model="model"
                        name="structRdi">
                    <label :for="'struct'+item.id" class="name">{{item.name}}</label>
                </li>
            </ul>
        </transition>
    </div>
      ...
      return Object.assign({},
        ...
        // 或覆写一些监听器的行为
        {
          ...
          change: function (event) {
            ...
            vm.$emit('structChange', value, name)
          }
        }
      )
    }
  ...
  methods: {
    getDicCommons () {
      api.getDicCommon('DicMonomerStructType').then(res => {
        ...
        this.struct.unshift({ 'id': '00000', 'name': '不限' })
      })
    }
  },
  mounted () {
