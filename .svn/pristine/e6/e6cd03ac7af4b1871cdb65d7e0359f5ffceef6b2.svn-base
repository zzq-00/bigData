...
<template>
  <div class="table">
    <el-table  :data="vm.tableData" style="width: 100%"
              :max-height="maxHeight" empty-text="无更多数据！" row-key="id"
      ...
      <el-table-column v-for="(item,index) in proList" :key="index" :label="item.name" align="center">
        ...
        <el-table-column label="" width="70" align="center" v-if="sign==2||sign==3">
          ...
          <template slot-scope="scope" v-if="sign==3||sign==2">
            ...
            <el-checkbox v-model="scope.row['calc-'+item.id]" @change="changevalue(scope.row,item.id,'calc')" v-if='sign==3'></el-checkbox>
          </template>
        </el-table-column>
        <el-table-column :prop="'amount-'+item.id" label="工程量" width="" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <span :class="['data-span',{'red-color':`${scope.row['result-'+item.id]}`==1},{'underline-color':`${scope.row['result-'+item.id]}`==2}]">{{scope.row['result-'+item.id]==1?'漏项':`${scope.row['amount-'+item.id]}`}}</span>
...
export default {
  ...
  props: ['maxHeight', 'abnormal', 'monomers', 'sign', 'tableDatas'],
  data() {
    return {
        config:{dataType:"3"},
        proList: [], //工程名字列表
